...
namespace aoc_utils {
    ...
    template <typename T> struct matrix {
        ...
        long long row_sum(size_t r) const {
            check_bounds(r);
            return std::accumulate(&dat[r][0], &dat[r][ncol], (long long) 0);
        }
      ...
      private:
        void check_bounds(size_t r, size_t c = 0) const {
            if (r >= nrow)
                throw std::out_of_range("Vector row index out of bounds" +
                                        std::to_string(r));
            if (c >= ncol)
                throw std::out_of_range("Vector column index out of bounds" +
                                        std::to_string(c));
        }
    };
} // namespace aoc_utils
